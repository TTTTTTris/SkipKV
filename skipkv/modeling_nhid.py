...
from transformers.models.qwen3.modeling_qwen3 import Qwen3RMSNorm, Qwen3Config, Qwen3ForCausalLM, Qwen3Model
...
def CausalLM_init(self, config):
    ...
    self.label = {}
    self.num_sentences = []
    self._accumulated_hidden_states = [] # Keep for current step processing
...
def Qwen3ForCausalLM_init(self, config):
    super(Qwen3ForCausalLM, self).__init__(config)
    self.model = Qwen3Model(config)
    self.vocab_size = config.vocab_size
    self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias=False)
...
def check_punct_tokens_in_forward(self, input_ids=None, padding_mask=None): 
    ...
    if input_ids is not None:
        ...
        if not hasattr(self, '_accumulated_hidden_states') or len(self._accumulated_hidden_states) != batch_size:
            self._accumulated_hidden_states = [{} for _ in range(batch_size)]
...
def reset_for_new_sample(self):
    ...
    if self.monitor_punct_tokens:
        ...
        self.label = {}
        self.num_sentences = []
        self._accumulated_hidden_states = []
...
def extract_hidden_states_by_ranges(
    hidden_states, 
    sentence_ranges, 
    _accumulated_hidden_states,
    current_pos = 0
):
    """
    Store hidden-state slices for each (start, end) range into _accumulated_hidden_states.

    Args:
        hidden_states: Tensor [B, T, H]
        sentence_ranges: List[List[(start,end)]], per batch (end is inclusive by default)
        _accumulated_hidden_states: List[Dict[(start,end), Tensor]] per batch

    Returns:
        None (writes into _accumulated_hidden_states in-place)
    """
    batch_size = hidden_states.shape[0]

    def process_single_batch(batch_idx, k, current_pos, batch_sentence_ranges, batched_hidden_states_dict):
        """Process a single batch and return results"""
        if k.numel() == 0:
            return
        ...
        if T > 1: # prefilling
            for start, end in batch_sentence_ranges:
                batched_hidden_states_dict[(start, end)] = k[start : end + 1]
            if end + 1 < current_pos:
                batched_hidden_states_dict[(end + 1, current_pos - 1)] = k[end + 1:]
        
        else: # decoding
            start, end = batch_sentence_ranges[-1]
            (s, e), last_value = batched_hidden_states_dict.popitem()
            if s == start:
                if e == end:
                    batched_hidden_states_dict[(s, e)] = last_value
                    batched_hidden_states_dict[(end + 1, current_pos - 1)] = k
                else:
                    batched_hidden_states_dict[(start, end)] = torch.cat([last_value, k], dim = 0)
            elif s > start:
                batched_hidden_states_dict[(s, e + 1)] = torch.cat([last_value, k], dim = 0)
            else:
                batched_hidden_states_dict[(s, e)] = last_value
                batched_hidden_states_dict[(start, end)] = k
            
        for (s, e), hs in batched_hidden_states_dict.items():
            if hs is not None:
                assert hs.size(0) == (e - s + 1), \
                    f"[curr pos {current_pos}, batch {batch_idx}] Corrupted range {(s, e)}: hs_len={hs.size(0)} expected={e - s + 1}"

    # Multi-batch processing
    if sentence_ranges:
        for batch_idx in range(batch_size):
            k = hidden_states[batch_idx]  # [seq_len, hidden_dim]
            batch_sentence_ranges = sentence_ranges[batch_idx]
            process_single_batch(batch_idx, k, current_pos[batch_idx], 
                                    batch_sentence_ranges, _accumulated_hidden_states[batch_idx])
...
def extract_sentence_representations(
    ...
    """
    batch_size = len(sentence_reprs_norm)

    def process_single_batch(last_token_pos, batch_accumulated_hidden_states, batch_sentence_reprs_norm):
        """Process a single batch and return results"""
        for (start_idx, end_idx), sentence_tokens in batch_accumulated_hidden_states.items():
            if sentence_tokens is None:
                continue

            if end_idx > last_token_pos: # avoid computing the repr uncompleted sentence range
                continue
            ...
            batch_accumulated_hidden_states[(start_idx, end_idx)] = None

    
    # Multi-batch processing
    if any(_accumulated_hidden_states):
        for batch_idx in range(batch_size):
            process_single_batch(last_token_pos[batch_idx], _accumulated_hidden_states[batch_idx], sentence_reprs_norm[batch_idx])
...
def cal_sentence_similarity_from_reprs(
    ...
    def process_single_batch(sentence_reprs: dict, new_range_start: int):
        """Compute redundancy using NEW (start > new_range_start) vs ALL sentence representations.
        ...
        # Identify new sentences whose start index >= new_range_starts
        new_indices = [i for i, r in enumerate(all_ranges) if r[0] > new_range_start]
    ...
    if isinstance(sentence_reprs_norm, list):
        ...
        for b, batch_reprs in enumerate(sentence_reprs_norm):
            rmap, smat = process_single_batch(batch_reprs, new_range_starts[b])
            range_to_similarity[b].update(rmap)
            sim_mats.append(smat)
        return range_to_similarity, sim_mats
    else:
        rmap, smat = process_single_batch(sentence_reprs_norm, new_range_starts)
...
def CausalLM_forward(
    ...
    **kwargs,
    ...
    extract_hidden_states_by_ranges(hidden_states, self.completed_punct_ranges, self._accumulated_hidden_states, self.current_position)
    ...
    if is_newline and hasattr(self.config, 'method'):
        if self.config.method in ['skipkv']:
            # Multi-batch processing
            last_token_pos = [0] * batch_size
            if any(self.labeled_sentence_ranges_per_batch):
                last_token_pos = [s[-1][0][-1] for s in self.labeled_sentence_ranges_per_batch]
            new_range_starts = [0] * batch_size
            if any(self._sentence_representations_per_batch):
                new_range_starts = [list(s.keys())[-1][-1] for s in self._sentence_representations_per_batch]
            extract_sentence_representations(self._accumulated_hidden_states, self._sentence_representations_per_batch, last_token_pos)

            _, batched_sentence_similarity = cal_sentence_similarity_from_reprs(
                self._sentence_representations_per_batch, 
                self._remove_punct_ranges_per_batch,
                new_range_starts,
            ...
            # logging.info(f'similarity: {batched_sentence_similarity}')
            # logging.info(f'removed ranges: {self._remove_punct_ranges_per_batch}')
            ...
            ### view removed sentences ###
            if self.config.method_config['record_kept_token_indices'] and self._remove_punct_ranges_per_batch:
                # Handle multi-batch debugging
                if hasattr(self, "completed_punct_ranges_per_batch") and self.completed_punct_ranges_per_batch:
                    batch_idx = 0
                    if self._remove_punct_ranges_per_batch[batch_idx]:
                        remove_punct_ranges = self._remove_punct_ranges_per_batch[batch_idx] if batch_size > 1 else self._remove_punct_ranges_per_batch
